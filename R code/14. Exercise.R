...
plot_Power <- generate_plot(data, "Watt_high", "bold(Power[high]*' (W)')", seq(0,  600, by = 25))
plot_HR <- generate_plot(data, "HR_high", "bold(HR[high] * ' (min'^{-1} * ')')", seq(0,  600, by = 10))


# Combine the plots using cowplot
combined_plot <- plot_grid(plot_Power, plot_HR, ncol = 1, align = "v")
...
ggsave("Exercise.png", final_plot, dpi = 900, width = 9, height = 9/4)
