...
plot_PAPm_rest <- generate_plot(data, "PAPm_rest", "bold(PAPm[rest]*' (mmHg)')", seq(0,  25, by = 5))
plot_PAPm_high <- generate_plot(data, "PAPm_high", "bold(PAPm[high]*' (mmHg)')", seq(10, 60, by = 5))
plot_PAPmCO    <- generate_plot(data, "PAPmCO",    "bold(PAPm/CO*' (WU)')",      seq(0,  18, by = 3), c(0, 18))
plot_VC        <- generate_plot(data, "VC",        "bold(VC*' (%)')",      seq(0,   700, by = 100), c(0, 700))
...
ggsave("Hemodynamic variables.png", final_plot, dpi = 900, width = 9, height = 4.5)
